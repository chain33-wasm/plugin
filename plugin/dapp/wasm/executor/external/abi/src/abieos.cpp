...
extern "C" const char *genAbiFromJson(const char *contractName, 
	const char *actionName, 
	const char *contractAbi,
	const char *abijson,
	int *pAbisize) {

    abieos_context *ctx = abieos_create();
	const char *result = handle_exceptions(ctx, nullptr, [&]() -> const char* {
		uint64_t contract = abieos_string_to_name(ctx, contractName);
	    uint64_t action_name = abieos_string_to_name(ctx, actionName);
	    if (!abieos_set_abi(ctx, contract, contractAbi)) {
	        std::cout << "abieos_set_abi error \n";
	        return nullptr;
	    }
	    const char* type = abieos_get_type_for_action(ctx, contract, action_name);
		
		abieos_json_to_bin(ctx, contract, type, abijson);
#if 0
		std::cout << "genAbiFromJson "<<abieos_get_bin_hex(ctx) << '\n';
#endif

		int binsize = abieos_get_bin_size(ctx);
...
#if 0
        const char *bindata222 = abieos_get_bin_data(ctx);
		for (int i = 0; i < binsize; i++) {
		    printf("%X",bindata222[i]);

		}
		printf("\n");
		

        for (auto iter = ctx->result_bin.begin(); iter != ctx->result_bin.end(); iter++) {
			printf("%02X",char(*iter));
		...
		memcpy(bindata, abieos_get_bin_data(ctx), binsize);

		*pAbisize = binsize;
		return (const char *)bindata;
	});
