...
type MemoryStateDB struct {
	...
	// 存储makeLogN指令对应的日志数据
	logs    map[common.Hash][]*loccom.ContractLog
	logSize uint
...
func NewMemoryStateDB(StateDB db.KV, LocalDB db.KVDB, CoinsAccount *account.DB, blockHeight int64) *MemoryStateDB {
	mdb := &MemoryStateDB{
		...
		logs:         make(map[common.Hash][]*loccom.ContractLog),
...
func (self *MemoryStateDB) GetAbi(addr string) []byte {
	acc := self.GetAccount(addr)
	if acc != nil {
...
func (self *MemoryStateDB) SetCodeAndAbi(addr string, code []byte, abi []byte) {
	acc := self.GetAccount(addr)
	if acc != nil {
...
// SSTORE 指令修改合约状态数据
func (self *MemoryStateDB) SetState(addr, key string, value []byte) {
	acc := self.GetAccount(addr)
	if acc != nil {
...
func (self *MemoryStateDB) GetChangedData(version int, opType loccom.WasmContratOpType) (kvSet []*types.KeyValue, logs []*types.ReceiptLog) {
	if version < 0 {
		return
	}

	for _, snapshot := range self.snapshots {
		kv, log := snapshot.getData(opType)
		if kv != nil {
...
func (self *MemoryStateDB) AddLog(log *loccom.ContractLog) {
	self.addChange(addLogChange{txhash: self.txHash})
	log.TxHash.SetBytes(self.txHash.Bytes())
