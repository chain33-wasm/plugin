...
//std::map<Name, Function*>& Module::getFunctionMap() {
    //return functionsMap;
//}
...
Function* Module::getFunction(Name name) {
	///////////////debug code//////////////////
#if 0
	std::cout<<"Function name in map is as below:" << "\n";
	for (std::map<Name, Function*>::iterator it = functionsMap.begin(); 
	    it != functionsMap.end(); it++) {
        std::cout<<"Name is:" <<it->first.c_str() << ", function name:" << it->second->name.c_str() << "\n";
	}
#endif		
	//////////////////////////////////////////
...
void Module::updateMaps() {
  ...
  std::unique_ptr<class wasmLogWriter> getwasmLogWriter(void){
		std::unique_ptr<class wasmLogWriter> up(new wasmLogWriter);
		return up;
	}

	std::unique_ptr<class wasmLogWriter> pwasmLogWriter;
	void writewasmRunLog(const char *msg) {
		static int initAlready = 0;
	    if (0 == initAlready) {
			initAlready = 1;
			pwasmLogWriter = getwasmLogWriter();
		}
		pwasmLogWriter->writeMsg(msg);
	}

	void writewasmRunLogStr(std::string msg) {
		writewasmRunLog(msg.c_str());
	}
