...
namespace fc
{
   ...
   {
      writewasmRunLog(m.get_message().c_str());
   ...
   unique_ptr<class wasmLogWriter> getwasmLogWriter(void){
		unique_ptr<class wasmLogWriter> up(new wasmLogWriter);
		return up;
	}

	unique_ptr<class wasmLogWriter> pwasmLogWriter;
