...
#if 0
         std::cout << "\n\n\n---------------------------------\n";
         std::cout << "add_import with func_name:" << func_name <<" \n";
		 std::cout << "Already existed func with names is as below:"  <<" \n";
		 if (module.functions.imports.size() != 0) {
		 	for (int i= 0; i < module.functions.imports.size(); i++) {
				std::cout << "Already existed func with names:" << module.functions.imports[i].moduleName <<" "
					" "<< module.functions.imports[i].exportName << " \n";
   ...
   class wasm_binary_injection {
      ...
      public:
         ...
         void inject() {
            ...
            // inject check_gas() first
            injector_utils::add_import<ResultType::none>( *_module, u8"check_gas", checktime_injection::chktm_idx );
