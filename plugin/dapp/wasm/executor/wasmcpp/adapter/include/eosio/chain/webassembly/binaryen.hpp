...
struct intrinsic_registrator {
   ...
   static auto& get_gasmap() {
       static map<intrinsic_fn, int> _gasmap;
	   return _gasmap;
   }

   static int get_gascost4func(intrinsic_fn fn) {
   	   //void *realfn = static_cast<(void *)>(fn);
       std::map<intrinsic_fn, int>::iterator iter = get_gasmap().find(fn);
	   if (iter == get_gasmap().end()) {
           EOS_ASSERT(false, wasm_execution_error, "can't get gas for function with addr");
		   return -1;
	   }
	   return iter->second;
   }

   intrinsic_registrator(const char* name, intrinsic_fn fn, int gas = 2)
   {
      //void *realfn = static_cast<(void *)>(fn);
      get_map()[string(name)] = fn;
	  get_gasmap()[fn] = gas;
	  //printf("register intrinsic func with name:%s, intrinsic_fn:0x%x with gas:%d \n", 
	  	//name, fn, gas);
   }
};
...
{
   std::string begin = fc::format_string("array_ptr_impl with length:${a}.\n",
		fc::mutable_variant_object()("a", length));
...
struct intrinsic_function_invoker {
   ...
   static Ret wrapper(interpreter_interface* interface, Params... params, LiteralList&, int) {
      //void * pmethod = (void *)&(class_from_wasm<Cls>::value(interface->context).*Method);
	  class_from_wasm<Cls>::value(interface->context).check_gas();
