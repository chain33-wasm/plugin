...
#include "../builtins/compiler_builtins.hpp"
...
namespace eosio { namespace chain {
   ...
   //modified by hezhengjun on 2018.9.15
   ...
   void wasm_interface::apply(const digest_type& code_id, const char *pcode, int code_size, apply_context& context) {
      my->get_instantiated_module(code_id, pcode, code_size)->apply(context);
   }
...
class context_aware_api {
   public:
	  ...
	  void check_gas() {
		int gas = 2;
        if (gas > 0) {
            context.check_and_spend_gas(gas);
			return;
		}
		EOS_ASSERT( gas > 0 , unaccessible_api , "unregister function without setting gas" );
		return;
	  }
...
class system_api : public context_aware_api {
   public:
      ...
      //Not implemented now, if required, realized it later
...
class action_api : public context_aware_api {
   ...
   action_api( apply_context& ctx )
      ...
      int read_action_data(array_ptr<char> memory, size_t buffer_size) {	  	
         auto s = context.actInfo.data.size();
         if( buffer_size == 0 ) return s;

         auto copy_size = std::min( buffer_size, s );
         memcpy( memory, context.actInfo.data.data(), copy_size );

         return copy_size;
      }

      int action_data_size() {	
         return context.actInfo.data.size();
      }
...
class console_api : public context_aware_api {
   public:
      ...
      void prints_l(array_ptr<const char> str, size_t str_len ) {
         if ( !ignore ) {
            context.console_append(std::string(str, str_len));
         }
      }
      ...
      void printi128(const __int128& val) {
         if ( !ignore ) {
			...
			fc::uint128_t v;
			v.hi = val_magnitude>>64;
			v.lo = static_cast<uint64_t>(val_magnitude);
	  ...
	  void output2user(null_terminated_ptr type, array_ptr<const char> data, size_t data_len) {
	      context.output2user(type, data, data_len);
	  }

#if 0
   void setOutType(null_terminated_ptr type) {
			 context.setOutType(type);
		 }
   
		 void setOutValue(array_ptr<const char> data, size_t data_len) {
			 context.setOutValue(data, data_len);
		 }
#endif
...
class database_api : public context_aware_api {
	public:
		//using context_aware_api::context_aware_api;
	    database_api( apply_context& ctx )
        :context_aware_api(ctx,true){}

		void pass_key(array_ptr<const char> key, size_t buffer_size) {
		    context.setKey(key, buffer_size);
			return;
		}

		void set_value(array_ptr<const char> value, size_t buffer_size) {
			context.setValue(value, buffer_size);
			return;
		}

		int get_value(array_ptr<char> value, size_t buffer_size) {
			return context.getValue(value,buffer_size);
		}
...
REGISTER_INJECTED_INTRINSICS(context_aware_api,
   (check_gas,      void())
);
